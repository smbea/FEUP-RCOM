...
int main(int argc, char** argv) {
	...
	Ftp *ftp;
	if(address.user[0] == '\0')
		ftp = ftp_init(address.host, NULL, NULL, address.path, address.filename);
	else 
		ftp = ftp_init(address.host, address.user, address.password, address.path, address.filename);
